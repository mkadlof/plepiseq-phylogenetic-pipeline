...
def main(input_metadata: str, output_metadata: str, features: tuple) -> None:
    ...
    with open(output_metadata, 'w') as output_f:
        for _, row in metadata.iterrows():
            ...
            # ensure that we have at least one feature to query
            if not any(row.get(feature) for feature in features):
                # throw an error
                raise ValueError("No valid geographic features found in the row.")
