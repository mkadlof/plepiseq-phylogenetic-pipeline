process create_input_params_json {
    ...
    if [[ ${genus} =~ "\${bacteria}" ]]; then
        ...
        'input_data' : '`realpath ${ExecutionDir}/${params.input_dir}`',
        'input_metadata' : '`realpath ${ExecutionDir}/${params.metadata}`',
        'results_dir' : '`realpath ${ExecutionDir}/${params.results_dir}`/${params.results_prefix}',
